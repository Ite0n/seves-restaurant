  ...
  const renderCard = (
    ...
    <article
      ...
      className={`group relative flex shrink-0 flex-col overflow-hidden rounded-sm bg-ink-900/60 ring-1 ring-gold/12 transition-shadow duration-700 ${
        variant === "scroll"
          ? "experience-card-glow w-[86vw] lg:w-[72vw] xl:w-[64vw]"
          : "experience-card-glow w-full"
      }`}
      data-cursor="hover"
    >
      <div className="relative aspect-[16/10] min-h-[12rem] w-full overflow-hidden md:aspect-[21/10]">
        <div
        ...
        <span className="absolute left-6 top-6 z-[4] font-display text-[0.65rem] tracking-[0.3em] text-gold/80">
          0{i + 1}
        </span>
      </div>

      <div className="relative z-[4] flex flex-1 flex-col p-7 md:p-9 lg:p-10">
        <span className="text-[0.6rem] uppercase tracking-luxe text-gold/75">
          {exp.subtitle}
        </span>
        <h3 className="mt-2 font-serif text-3xl text-cream md:text-4xl">{exp.title}</h3>
    ...
    <section id="experiences" className="relative overflow-hidden bg-ink-800">
      ...
      <div ref={scrollContainerRef} className="relative">
        <div className="flex h-[100svh] flex-col justify-center px-6 py-14 lg:px-10">
          ...
          <div className="relative mt-10 h-[58vh] min-h-[22rem] overflow-hidden">
            <div
              ref={trackRef}
              className="flex h-[58vh] min-h-[22rem] items-stretch gap-6 pr-[14vw] will-change-transform lg:gap-8"
            >
